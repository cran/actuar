\documentclass{article}
  ...
  \title{Risk theory features of \pkg{actuar}}
...
The current version of \pkg{actuar} contains four visible functions
related to the above problems: two for the calculation of the
aggregate claim amount distribution and two for ruin probability
calculations. We feel the implementations make \textsf{R} shine as a
computing and modeling platform for risk theory.
...
Let the random variable $S$ represent the aggregate claim amount (or
...
identically distributed random variables each independent from $N$.
...
distribution (see Subsection \ref{sec:aggregate}) require a
...
Figure \ref{fig:discretization-methods} illustrates the four methods.
...
Usage of \code{discretize} is similar to \proglang{R}'s plotting
...
\item Recursive calculation using the algorithm of \cite{Panjer_81}.
  ...
  contain the Poisson, binomial and negative binomial distributions
  and their extensions with an arbitrary mass at $x = 0$.) The general
  ...
  computed until their sum is close to 1 (how close is the user's
  choice).

  The recursions are done in \proglang{C} to dramatically increase
...
\item Exact calculation by numerical convolutions using
  \eqref{eq:cdf-S} and \eqref{eq:convolution-formula}. Hence, this
  ...
  computed with \proglang{R}'s function \code{convolve}. This approach
...
\item Simulation of a random sample from $S$ and approximation of
  $F_S(x)$ by the empirical cdf. The simulation itself is done with
  function \code{simpf} (see the \code{"credibility"} vignette)).
  This function admits very general hierarchical models for both the
  frequency and the severity components.
\end{enumerate}

Here also, adding other methods to \code{aggregateDist} is simple due
to its modular conception.

The arguments of \code{aggregateDist} differ depending on the
...
\code{CTE} with methods for \code{"aggregateDist"} objects. The
former computes the value-at-risk $\VaR_\alpha$ such that
\begin{equation}
  \label{eq:VaR}
...
To conclude on the subject, Figure \ref{fig:Fs-comparison} shows the
...
We define some other quantities of interest in the sequel. Let $N(t)$
...
\item the sequence $\{T_j\}_{j \geq 1}$ forms an ordinary renewal
  process, with the consequence that the random variables $W_1, W_2,
...
The quantity known as the adjustment coefficient $R$ hardly has any
physical interpretation, but it comes useful as an approximation to
the probability of ruin since we have the inequality
\begin{displaymath}
  \psi(u) \leq e^{-R u}, \quad u \geq 0.
\end{displaymath}
...
coefficient $R$ from the following arguments: either the two moment
generating functions $M_C(t)$ and $M_W(t)$ (thereby assuming
independence) or else function $h(t)$; the premium rate $c$; the upper
bound of the support of $M_C(t)$ or any other upper bound for $R$.
...
<<echo=TRUE>>=
adjCoef(mgf.claim = mgfexp(x), mgf.wait = mgfexp(x, 2), p = 2.4, upper = 1)
@
...
Figure \ref{fig:adjcoef} for the graph):
<<echo=TRUE, fig=FALSE>>=
mgfx <- function(x, y) mgfexp(x * y)
p <- function(x) 2.6 * x - 0.2
R <- adjCoef(mgfx, mgfexp(x, 2), premium = p, upper = 1, reins = "prop",
              from = 0, to = 1)
R(c(0.75, 0.8, 0.9, 1))
plot(R)
@
...
Phase-type$(\pmb{\pi}, \mat{T})$ distributed, then $\psi(u) = 1
- F(u)$, where $F$ is Phase-type$(\pmb{\pi}_+, \mat{Q})$ with
\begin{equation}
  \label{eq:prob-ruin:cramer-lundberg}
  \begin{split}
    \pmb{\pi}_+
    &= - \frac{\lambda}{c}\, \pmb{\pi} \mat{T}^{-1} \\
    \mat{Q}
    &= \mat{T} + \mat{t} \pmb{\pi}_+,
  \end{split}
\end{equation}
...
Phase-type$(\pmb{\pi}_+, \mat{Q})$, but with parameters
\begin{equation}
  \label{eq:prob-ruin:sparre:pi+}
  \pmb{\pi}_+  = \frac{\mat{e}^\prime (\mat{Q} - \mat{T})}{%
    c \mat{e}^\prime \mat{t}}
\end{equation}
...
Function \code{ruin} of \pkg{actuar} returns a function object to
compute the probability of ruin for any initial surplus $u$. In all
cases except the exponential/exponential model where
\eqref{eq:ruin-cramer-lundberg} is used, the output object calls
...
One can plot the probability of ruin function using \code{curve}
(see Figure \ref{fig:prob-ruin}):
<<echo=TRUE, fig=FALSE>>=
psi <- ruin(claims = "p", par.claims = list(prob = prob, rates = rates),
     wait   = "e", par.wait   = list(rate = c(5, 1), weights = c(0.4, 0.6)))
curve(psi, from = 0, to = 50, main = "Probability of ruin",
...
and upper bounds are (see figure \ref{fig:beekman:prob-ruin} for the
graphic):
<<echo=TRUE>>=
