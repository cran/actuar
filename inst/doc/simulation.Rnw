...
\usepackage[utf8]{inputenc}
  ...
  %% Sweave environments redefined to have a color background
  %% (following https://tex.stackexchange.com/a/141128).
...
R expressions consisting in calls to a random number
...
method to describe these models in R that will meet the
following criteria:
\begin{enumerate}
