...
the random variable $\Lambda$ is inverse Gaussian distribution with
...
\item Add an entry \samp{import(actuar)} in the \file{NAMESPACE} file
  of \pkg{pkg};
...
\begin{equation*}
  \frac{B^{-(x - \frac{1}{2})} K_{x - \frac{1}{2}}(B/\phi)}{x!}
\end{equation*}
