...
purposes. Package \pkg{actuar} offers many functions
related to loss distributions modeling. The
...
but including the loggamma distribution \citep{HoggKlugman}.
...
\begin{table}
  \begin{threeparttable}
    ...
    \begin{tabularx}{\textwidth}{lXl}
      ...
      \midrule
      Other             & Loggamma\tnote{1} & \code{lgamma} \\
...
is based on \code{expm()} from package \pkg{Matrix} \citep{Matrix}.
