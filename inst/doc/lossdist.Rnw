\documentclass{article}
  ...
  %\VignetteIndexEntry{Loss distributions modeling features}
...
One important task of actuaries is the modeling of claim amounts
distributions for ratemaking, loss reserving or other risk evaluation
purposes. Package \pkg{actuar} offers a large number of functions
related to loss distributions modeling. Some complement the many tools
already part of base \proglang{R}, while others provide support for
entirely untouched procedures common in Actuarial Science. The
following subsections detail the following \pkg{actuar} features:
\begin{enumerate}
\item introduction of 18 additional probability laws and functions to
  get raw moments, limited moments and the moment generating function;
...
\code{foo}, the functions are named \code{dfoo}, \code{pfoo},
...
already present in base R, excluding the inverse Gaussian and log-$t$
but including the loggamma distribution \citep{HoggKlugman}. For
reference, Appendix \ref{sec:appendix} lists the pdf and cdf of these
distributions.

Tables %
\ref{tab:distributions:trbeta_family}--\ref{tab:distributions:other} %
list the supported distributions classified by families. Each table
details the names of the distributions as given in
\cite{LossModels2e}, the root name of the \proglang{R} functions and the
names of the arguments corresponding to each parameter in the
parametrization of \cite{LossModels2e}. One will note that by default
all functions (except those for the Pareto distribution) use a rate
parameter equal to the inverse of the scale parameter. This differs
from \cite{LossModels2e} but is better in line with the functions for
the gamma, exponential and Weibull distributions in base \proglang{R}.

\begin{table}
  \caption{Supported distributions from the Transformed Beta family,
    root name of the \proglang{R} functions and argument name
    corresponding to each parameter in the parametrization of
    \cite{LossModels2e}.}
  \label{tab:distributions:trbeta_family}

  \begin{tabularx}{\textwidth}{XXXX}
    \toprule
    Distribution name & Root (alias) & Arguments \\
    \midrule
    Transformed beta & \code{trbeta} (\code{pearson6}) &
      \code{shape1} ($\alpha$) \newline
      \code{shape2} ($\gamma$) \newline
      \code{shape3} ($\tau$) \newline
    ...
    Generalized Pareto & \code{genpareto} &
      ...
      \code{scale}  ($\theta$) \\
      \midrule
    Pareto & \code{pareto} (\code{pareto2}) &
      \code{shape}  ($\alpha$) \newline
      \code{scale}  ($\theta$) \\
      \midrule
...
\begin{table}
  \caption{Supported distributions from the Transformed Gamma family,
    root name of the \proglang{R} functions and argument name
    corresponding to each parameter in the parametrization of
    \cite{LossModels2e}.}
  \label{tab:distributions:trgamma_family}

  \begin{tabularx}{\textwidth}{XXX}
    \toprule
    Distribution name & Root (alias) & Arguments \\
    \midrule
    Transformed gamma & \code{trgamma} &
      \code{shape1} ($\alpha$), \newline
      \code{shape2} ($\tau$) \newline
      \code{rate}   ($\lambda = 1/\theta$), \newline
      \code{scale}  ($\theta$), \\
      \midrule
    Inverse transformed\newline gamma & \code{invtrgamma} &
      \code{shape1} ($\alpha$), \newline
      \code{shape2} ($\tau$) \newline
      \code{rate}   ($\lambda = 1/\theta$), \newline
      \code{scale}  ($\theta$), \\
    ...
    Inverse gamma & \code{invgamma} &
      ...
      \code{scale}  ($\theta$) \\
      \midrule
    Inverse Weibull & \code{invweibull} (\code{lgompertz}) &
      \code{shape}  ($\tau$), \newline
      \code{rate}   ($\lambda = 1/\theta$), \newline
      \code{scale}  ($\theta$) \\
      \midrule
...
\begin{table}
  \begin{threeparttable}
    \caption{Other supported distributions, root name of the
      \proglang{R} functions and argument name corresponding to each
      parameter in the parametrization of \cite{LossModels2e}.}
    \label{tab:distributions:other}

    \begin{tabularx}{\textwidth}{XXX}
      \toprule
      Distribution name & Root (alias) & Arguments \\
      \midrule
      Loggamma\tnote{1} & \code{lgamma} &
        \code{shapelog} ($\alpha$) \newline
        \code{ratelog}  ($\lambda$) \\
        \midrule
      Single parameter\newline Pareto & \code{pareto1} &
        \code{shape} ($\alpha$) \newline
        \code{min}    ($\theta$) \\
        \midrule
...
\begin{equation}
  ...
  M_X(t) = \E{e^{tX}},
\end{equation}
when it exists. Every probability law of Tables %
\ref{tab:distributions:trbeta_family}--\ref{tab:distributions:other} %
is supported, plus the following ones: beta, exponential, chi-square,
gamma, lognormal, normal (except \code{lev}), uniform and Weibull of
base \proglang{R} and the inverse Gaussian distribution of package
...
matching of raw or limited moments; see Subsection
\ref{sec:empirical-moments} for their empirical counterparts. The
\code{mgf} functions come in handy to compute the adjustment
coefficient in ruin theory; see the \code{"risk"} vignette.

In addition to the 17 distributions of Tables %
\ref{tab:distributions:trbeta_family}--\ref{tab:distributions:other}, %
...
\end{equation}
is the \emph{matrix exponential} of matrix $\mat{M}$.
...
in \proglang{C} for speed. In almost every respect, they behave just
like the base \proglang{R} functions. The matrix exponential
\proglang{C} routine is a simplified version of \code{expm()} from
package \pkg{Matrix} \citep{Matrix}.
...
somewhat fallen out of fashion, to the point that the material on
grouped data present in the first edition of \cite{LossModels} has
been deleted in the second edition \cite{LossModels2e}.
...
replacement and summary functions, manipulation of grouped data
becomes similar to that of individual data.

First, function \code{grouped.data} creates a grouped data object
similar to --- an inheriting from --- a data frame. The input of the
...
Finally, the package defines methods of a few existing summary
functions for grouped data objects. Computing the mean
\begin{equation}
...
Subsection \ref{sec:empirical-moments}.
...
to individual data is well covered by function \code{fitdistr} of
...
deductible, a limit, coinsurance or inflation \cite[see][Chapter 5 for
precise definitions of these terms]{LossModels2e}. Table
\ref{tab:coverage} summarizes the definitions of $Y^L$ and $Y^P$.
...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{lll}
      \toprule
      Coverage modification & Per-loss variable &
      Per-payment variable \\
      \midrule
      Ordinary deductible ($d$)  & $\begin{cases} 0, &\; X \leq
        d\\ X - d, &\; X > d\end{cases}$ & $\begin{cases} \text{undefined}, &\; X \leq
        d\\ X - d, &\; X > d\end{cases}$ \medskip \\
      Franchise deductible ($d$) &  $\begin{cases} 0, &\; X \leq
        d\\ X, &\; X > d\end{cases}$ &  $\begin{cases} \text{undefined}, &\; X \leq
        d\\ X, &\; X > d\end{cases} $ \medskip \\
      Limit ($u$) & $\begin{cases} X, &\; X \leq
        u\\ u, &\; X > u\end{cases}$ & $\begin{cases} X, &\; X \leq
        u\\ u, &\; X > u\end{cases}$ \bigskip \\
  ...
  \end{center}
\end{table}
Often, one will want to use data $Y^L_1, \dots, Y^L_n$ (or $Y^P_1,
\dots, Y^P_n$) from the random variable $Y^L$ (or $Y^P$) to fit a
model on the unobservable random variable $X$. This requires to
express the pdf or cdf of $Y^L$ (or $Y^P$) in terms of the pdf or cdf
of $X$. Function \code{coverage} of \pkg{actuar} does just that: given
a pdf or cdf and any combination of the coverage modifications
mentioned above, \code{coverage} returns a function object to compute
the pdf or cdf of the modified random variable. The function can then
be used in modeling like any other \code{dfoo} or \code{pfoo}
function.
...
modifications submitted and contains no useless code. For comparison
purpose, the following function contains no deductible and no limit:
<<echo=TRUE>>=
g <- coverage(dgamma, pgamma)
g
@
...
in Tables
\ref{tab:distributions:trbeta_family}--\ref{tab:distributions:other}
using the parametrization of \cite{LossModels2e} and
\cite{HoggKlugman}.
