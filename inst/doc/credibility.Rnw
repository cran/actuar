...
The credibility theory facilities of \pkg{actuar} consist of matrix
\code{hachemeister} containing the famous data set of
\cite{Hachemeister_75} and function \code{cm} to fit hierarchical and
regression credibility models. Furthermore, function \code{simul} can
